...
// alternatively, we can use an accumulator for readibility
type WithoutWithAcc<T, U, Acc extends any[] = []> = T extends [
  infer F,
  ...infer Rest
]
  ? F extends (U extends number[] ? U[number] : U)
    ? WithoutWithAcc<Rest, U, [...Acc]>
    : WithoutWithAcc<Rest, U, [...Acc, F]>
  : Acc;
