...
type Absolute<T extends number | string | bigint> =
  `${T}` extends `-${infer WithoutSymbol}` ? WithoutSymbol : `${T}`;

//   Steps  are:
//   1. Convert the arguement to string since it can be a number too.
//   2. Once we have a string, check it contains a negative symbol. If it does, remove the symbol.
