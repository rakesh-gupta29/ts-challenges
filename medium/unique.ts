...
type Unique<T, Acc extends any[] = []> = T extends [infer F, ...infer Rest]
  ? IsElemInSet<F, Acc> extends true
    ? Unique<Rest, Acc>
    : Unique<Rest, [...Acc, F]>
  : Acc;
type IsElemInSet<E, S extends unknown[]> = S extends [
  infer Head,
  ...
  ? Equal<E, Head> extends true
    ...
    : IsElemInSet<E, Rest>
  : false;
// checking that the element extends  true using Equal method to handle data type equality

type Check = Unique<[string, number, 1, "a", 1, string, 2, "b", 2, number]>;
