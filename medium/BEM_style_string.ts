...
// this can be solved by creating a modifer options for each block
type Block<E> = E extends string ? `${E}` : never;
type Element<E extends string[]> = E[number] extends never
  ? ``
  : `__${E[number]}`;
type Modifier<E extends string[]> = E[number] extends never
  ? ``
  : `--${E[number]}`;
