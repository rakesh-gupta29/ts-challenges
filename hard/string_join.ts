...
type Join<
  Str extends string[],
  JoinBy extends string,
  Acc extends string = ""
> = Str extends [infer Elem, ...infer Rest]
  ? Join<
      Rest extends string[] ? Rest : never,
      JoinBy,
      `${Acc}${Acc extends "" ? "" : JoinBy}${Elem extends string
        ? Elem
        : never}`
    >
  : Acc;
declare function join<JoinBy extends string>(
  delimiter: JoinBy
): <Str extends string[]>(...parts: Str) => Join<Str, JoinBy>;
